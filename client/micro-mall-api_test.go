...
func TestSearchUserInfo(t *testing.T) {
	r := baseUrl + searchUserInfo + "?query=婷"
	t.Logf("request url: %s", r)
	req, err := http.NewRequest("GET", r, nil)
	if err != nil {
		t.Error(err)
...
func TestSearchSkuInventory(t *testing.T) {
	r := baseUrl + searchSkuInventory + "?keyword=纸巾"
	t.Logf("request url: %s", r)
	req, err := http.NewRequest("GET", r, nil)
	if err != nil {
		t.Error(err)
...
func TestOrderTradePay(t *testing.T) {
	...
	t.Logf("request url: %s", r)
	data := url.Values{}
	data.Set("tx_code", "da05e87b-e3bf-4370-9cec-b410c478eaf3")
...
func TestTradeCreateOrder(t *testing.T) {
	...
	t.Logf("request url: %s", r)
	goods1 := OrderShopGoods{
		SkuCode: "eb15ca0d-ccba-45f7-a46f-eb2fff5c323c",
		Price:   "23.78",
		Amount:  1,
		Name:    "百威啤酒",
		Version: 1,
	...
	goods2 := OrderShopGoods{
		SkuCode: "515ce053-baa7-401f-a0fd-e49de51bf101",
		Price:   "23.78",
		Amount:  2,
		Name:    "洁柔纸巾",
		Version: 1,
...
func TestRegisterUser(t *testing.T) {
	...
	data.Set("password", "98501707783")
	data.Set("sex", "1")
	data.Set("age", "33")
	data.Set("country_code", "86")
	data.Set("phone", "98501707783")
	data.Set("email", "565608463@qq.com")
	data.Set("verify_code", "129841")
	data.Set("id_card_no", fmt.Sprintf("10000000%d", time.Now().Unix()))
...
func TestSkuBusinessPutAway(t *testing.T) {
	...
	data := url.Values{}
	data.Set("operation_type", "0")
	data.Set("shop_id", "1000")
	data.Set("sku_code", uuid.New().String())
	data.Set("name", "洁柔纸巾")
	data.Set("price", "23.78")
	data.Set("title", "洁柔 整箱装")
	data.Set("sub_title", "洁柔 整箱装")
	data.Set("desc", "洁柔 12包 整箱装")
	data.Set("production", "洁柔纸巾")
	data.Set("supplier", "洁柔纸巾")
...
func TestGetSkuList(t *testing.T) {
	r := baseUrl + skuBusinessGetSkuList + "?shop_id=1000"
	t.Logf("request url: %s", r)
	req, err := http.NewRequest("GET", r, nil)
	if err != nil {
		t.Error(err)
